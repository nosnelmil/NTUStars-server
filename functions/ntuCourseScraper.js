...
async function extractContentData(contentPage) {
  ...
  const result = await contentPage.evaluate(async () => {
    ...
    const contentFrame = document.querySelector(
        ...
        "#top > div > section:nth-child(2) > div > div > p:nth-child(1) > table > tbody > tr > td.smallText2 > table > tbody > tr > td:nth-child(1) > iframe "
    );
    const rows = contentFrame.contentWindow.document.querySelectorAll("table > tbody tr");
    rows.forEach((row, index) => {
      const temp = [];
      const tdTags = row.querySelectorAll("td");
...
function parseRawContentData(rawContentData) {
  ...
  const possibleHeaders = new Set(
      ["Mutually exclusive with:", "Grade Type:", "Mutually exclusive with:",
        "Not available to Programme:", "Not available to all Programme with:",
        "Not offered as Unrestricted Elective"]
  );
  for (let i=2; i<rawContentData.length-1; i++) {
    const cur = rawContentData[i];
    switch (cur[0]) {
      case "Prerequisite:":
